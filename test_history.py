...
def test_request_history():
    ...
    # Adiciona mais requisições
    mock_flow.request.method = "POST"
    mock_flow.response.status_code = 201
    history.add_request(mock_flow)
    
    mock_flow.request.method = "PUT"
    mock_flow.response.status_code = 204
    history.add_request(mock_flow)
    
    assert len(history.get_history()) == 3, "Deve ter 3 entradas"
    print("✓ Múltiplas requisições adicionadas")
...
def test_intercept_addon_with_history():
    ...
    from intercept_proxy import InterceptConfig, InterceptAddon, RequestHistory
    from unittest.mock import Mock
