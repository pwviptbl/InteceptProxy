...
class RulesTab(QWidget):
    ...
    def _setup_form(self, layout):
        ...
        add_rule_layout.addWidget(add_button, 2, 0, 1, 4)
    ...
    def duplicate_rule(self):
        ...
        success, _ = self.config.add_rule(
            rule['host'], rule['path'], rule['param_name'], rule['param_value']
        )
        if success:
...
class RulesTableModel(QAbstractTableModel):
    ...
    def data(self, index, role):
        if role == Qt.ItemDataRole.DisplayRole:
            ...
            col = index.column()
            if col == 0: return rule.get('host', '')
            if col == 1: return rule.get('path', '')
            if col == 2: return rule.get('param_name', '')
            if col == 3: return rule.get('param_value', '')
            if col == 4: return "Ativo" if rule.get('enabled', True) else "Inativo"
        return None
